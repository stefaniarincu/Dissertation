...
def train_step(model, dataloader, optimizer, criterion, device):
    ...
    for batched_images, batched_masks in dataloader:
        ...
        # Freeze image encoder except adapters
...
if __name__ == '__main__':
    ...
    model = sam_model_registry['vit_b'](SAMArgs()).to(DEVICE)
    checkpoint = torch.load(SAM_CHECKPOINT_PATH, map_location=DEVICE, weights_only=False)
