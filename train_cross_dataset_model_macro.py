...
MODELS_AND_LOG_ROOT_PATH = '/root/Disertation/files/cross_dataset/biased/macro_loss_metrics'
...
class DiceBCELossPerSample(nn.Module):
    def __init__(self, weight=None, size_average=True):
        super(DiceBCELossPerSample, self).__init__()
    ...
    def forward(self, inputs, targets, smooth=1):
        ...
        bce_loss = F.binary_cross_entropy(inputs, targets, reduction='none').mean(dim=1)
        return bce_loss + dice_loss

def compute_feature_alignment_loss_per_sample(param_cross_dataset_features, param_dataset_specific_features, param_weights):
    batch_size = param_weights.shape[0]
    device = param_cross_dataset_features[0].device
    feature_alignment_loss_per_sample = torch.zeros(batch_size, device=device, dtype=torch.float32)
    ...
    for encoder_block_index in range(len(param_cross_dataset_features)):
        ...
        for dataset_id in range(len(DATASETS_TO_IDS)):
            ...
            feature_alignment_loss_per_sample += param_weights[:, dataset_id] * mse_difference_per_sample

    return feature_alignment_loss_per_sample

def compute_macro(param_values_per_sample, param_dataset_ids):
    unique_dataset_ids = param_dataset_ids.unique(sorted=False)
    datasets_means = []
    for dataset_id in unique_dataset_ids:
        datasets_means.append(param_values_per_sample[param_dataset_ids == dataset_id].mean())
    return torch.stack(datasets_means).mean()

def calculate_per_sample_metrics(y_true, y_pred, dataset_ids):
    ...
    y_true_sum = y_true.sum(dim=1)

    # Compute the scores for each metric
    union = y_true_sum + y_pred_sum - intersection
    score_precision = (intersection + 1e-15) / (y_pred_sum + 1e-15)
    score_recall = (intersection + 1e-15) / (y_true_sum + 1e-15)
    score_dice = (2.0 * intersection + 1e-15) / (y_true_sum + y_pred_sum + 1e-15)
...
def train_step(param_model, param_dataloader, param_optimizer, param_criterion, param_dataset_specific_models, param_weighting_mode, param_device):
    ...
    epoch_loss = 0.0
    metrics_sums_per_dataset = {dataset_id: [0.0, 0.0, 0.0, 0.0] for dataset_id in DATASETS_TO_IDS.values()}
    counts_per_dataset = {dataset_id: 0 for dataset_id in DATASETS_TO_IDS.values()}
    ...
    for batched_images, batched_masks, batched_dataset_ids in param_dataloader:
        ...
        dice_bce_loss_per_sample = param_criterion(y_pred, batched_masks)
        dice_bce_loss = compute_macro(dice_bce_loss_per_sample, batched_dataset_ids)
        ...
        with torch.no_grad():
            ...
            if param_weighting_mode == 0:
                ...
                feature_alignment_loss_per_sample = torch.zeros(batched_images.shape[0], device=batched_images.device, dtype=torch.float32)
                for encoder_block_index in range(len(cross_dataset_features)):
                    ...
                    feature_alignment_loss_per_sample += mse_difference
                feature_alignment_loss = compute_macro(feature_alignment_loss_per_sample, batched_dataset_ids)
            ...
            # Otherwise, the features are collected from all dataset specific models and weighted based on the specified mode to compute the feature alignment loss
                ...
                feature_alignment_loss_per_sample = compute_feature_alignment_loss_per_sample(cross_dataset_features, dataset_specific_features, dataset_specific_models_weights)
                feature_alignment_loss = compute_macro(feature_alignment_loss_per_sample, batched_dataset_ids)
        ...
        # Calculate metrics
        per_sample_metrics = calculate_per_sample_metrics(batched_masks, y_pred, batched_dataset_ids)
        for sample_id in range(batched_images.shape[0]):
            dataset_id = int(batched_dataset_ids[sample_id].item())
            for metric_id in range(4):
                metrics_sums_per_dataset[dataset_id][metric_id] += per_sample_metrics[metric_id][sample_id].item()
            counts_per_dataset[dataset_id] += 1

    epoch_loss /= len(param_dataloader)
    num_datasets = len(metrics_sums_per_dataset.keys())
    epoch_jaccard = sum(metrics_sums_per_dataset[dataset_id][0] / counts_per_dataset[dataset_id] for dataset_id in metrics_sums_per_dataset) / num_datasets
    epoch_dice = sum(metrics_sums_per_dataset[dataset_id][1] / counts_per_dataset[dataset_id] for dataset_id in metrics_sums_per_dataset) / num_datasets
    epoch_recall = sum(metrics_sums_per_dataset[dataset_id][2] / counts_per_dataset[dataset_id] for dataset_id in metrics_sums_per_dataset) / num_datasets
    epoch_precision = sum(metrics_sums_per_dataset[dataset_id][3] / counts_per_dataset[dataset_id] for dataset_id in metrics_sums_per_dataset) / num_datasets
...
def evaluate_step(param_model, param_dataloader, param_criterion, param_device):
    ...
    loss_sums_per_dataset = {dataset_id: 0.0 for dataset_id in DATASETS_TO_IDS.values()}
    metrics_sums_per_dataset = {dataset_id: [0.0, 0.0, 0.0, 0.0] for dataset_id in DATASETS_TO_IDS.values()}
    counts_per_dataset = {dataset_id: 0 for dataset_id in DATASETS_TO_IDS.values()}
    ...
    with torch.inference_mode():
        for batched_images, batched_masks, batched_dataset_ids in param_dataloader:
            ...
            metrics_per_sample = calculate_per_sample_metrics(batched_masks, y_pred, batched_dataset_ids)

            # Accumulate loss and metrics per dataset
            for sample_id in range(batched_images.shape[0]):
                dataset_id = int(batched_dataset_ids[sample_id].item())
                loss_sums_per_dataset[dataset_id] += dice_bce_loss_per_sample[sample_id].item()
                for metric_id in range(4):
                    metrics_sums_per_dataset[dataset_id][metric_id] += metrics_per_sample[metric_id][sample_id].item()
                counts_per_dataset[dataset_id] += 1

    num_datasets = len(metrics_sums_per_dataset.keys())
    epoch_loss = sum(loss_sums_per_dataset[dataset_id] / counts_per_dataset[dataset_id] for dataset_id in loss_sums_per_dataset) / num_datasets
    epoch_jaccard = sum(metrics_sums_per_dataset[dataset_id][0] / counts_per_dataset[dataset_id] for dataset_id in metrics_sums_per_dataset) / num_datasets
    epoch_dice = sum(metrics_sums_per_dataset[dataset_id][1] / counts_per_dataset[dataset_id] for dataset_id in metrics_sums_per_dataset) / num_datasets
    epoch_recall = sum(metrics_sums_per_dataset[dataset_id][2] / counts_per_dataset[dataset_id] for dataset_id in metrics_sums_per_dataset) / num_datasets
    epoch_precision = sum(metrics_sums_per_dataset[dataset_id][3] / counts_per_dataset[dataset_id] for dataset_id in metrics_sums_per_dataset) / num_datasets
...
if __name__ == '__main__':
    ...
    criterion = DiceBCELossPerSample()
