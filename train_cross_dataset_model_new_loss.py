...
# Function that saves the model state, optimizer state, scheduler state, current epoch, best validation metric, number of epochs with no improvement and the RNG states for reproducibility
def save_resume_checkpoint(param_model, param_training_loss_state, param_optimizer, param_scheduler, param_epoch, param_best_validation_metric, param_num_epochs_no_improvement, param_path=RESUME_CHECKPOINT_PATH):
    torch.save({
        'model_state': param_model.state_dict(),
        'training_loss_state': param_training_loss_state.state_dict(),
...
# Function that loads all necessary states from a checkpoint to resume training
def load_resume_checkpoint(param_model, param_training_loss, param_optimizer, param_scheduler, param_path=RESUME_CHECKPOINT_PATH, param_device=DEVICE):
    ...
    param_training_loss.load_state_dict(checkpoint['training_loss_state'])
...
# Balanced Batch Sampler that ensures that each batch contains a specified number of samples from each dataset
class BalancedBatchSampler(Sampler):
    def __init__(self, param_dataset_ids, param_batch_composition):
        ...
        self.epoch = 0
        self.indices_per_dataset = {dataset_id: np.where(self.dataset_ids == dataset_id)[0] for dataset_id in self.batch_composition.keys()}

        batches_per_dataset = []
        for dataset_id, num_samples_per_batch in self.batch_composition.items():
            num_samples = len(self.indices_per_dataset[dataset_id])
            num_batches = int(np.ceil(num_samples / num_samples_per_batch))
    ...
    def __iter__(self):
        ...
        for dataset_id in self.batch_composition.keys():
            dataset_indices = self.indices_per_dataset[dataset_id].tolist()
...
# TResUnet model with ResNet50 backbone and transformer bottleneck
...
# Segmentation loss that combines Dice loss and Binary Cross Entropy loss
...
class TotalLoss(nn.Module):
    def __init__(self, param_gamma, param_delta):
        ...
        self.segmentation_loss = DiceBCELoss()

    @staticmethod
    def compute_cosine_loss_pooled(param_cross_dataset_feature, param_dataset_specific_feature):
        cross_dataset_feature_pooled = F.adaptive_avg_pool2d(param_cross_dataset_feature, 1).flatten(1)
        dataset_specific_feature_pooled = F.adaptive_avg_pool2d(param_dataset_specific_feature, 1).flatten(1)

        cross_dataset_feature_normalized = F.normalize(cross_dataset_feature_pooled, p=2, dim=1)
        dataset_specific_feature_normalized = F.normalize(dataset_specific_feature_pooled, p=2, dim=1)

        return 1.0 - (cross_dataset_feature_normalized * dataset_specific_feature_normalized).sum(dim=1)
    
    def forward(self, param_cross_dataset_predictions, param_ground_truth_masks, param_cross_dataset_features, param_dataset_specific_features_by_dataset_id, param_dataset_specific_models_weights):
        segmentation_loss = self.segmentation_loss(param_cross_dataset_predictions, param_ground_truth_masks)
        batch_size, num_dataset_specific_models = param_dataset_specific_models_weights.shape

        mse_loss, cosine_loss = 0.0, 0.0

        for encoder_block_index in range(len(param_cross_dataset_features)):
            cross_dataset_feature = param_cross_dataset_features[encoder_block_index]

            mse_per_sample_loss = torch.zeros(batch_size, device=cross_dataset_feature.device)
            cosine_per_sample_loss = torch.zeros(batch_size, device=cross_dataset_feature.device)

            cross_dataset_feature_normalized = F.normalize(cross_dataset_feature, p=2, dim=1)

            for dataset_specific_model_index in range(num_dataset_specific_models):
                dataset_specific_feature = param_dataset_specific_features_by_dataset_id[dataset_specific_model_index][encoder_block_index].detach()

                dataset_specific_feature_normalized = F.normalize(dataset_specific_feature, p=2, dim=1)
                mse = F.mse_loss(cross_dataset_feature_normalized, dataset_specific_feature_normalized, reduction='none').mean(dim=(1, 2, 3))
                cosine = self.compute_cosine_loss_pooled(cross_dataset_feature, dataset_specific_feature)

                current_model_weights = param_dataset_specific_models_weights[:, dataset_specific_model_index]
                mse_per_sample_loss += current_model_weights * mse
                cosine_per_sample_loss += current_model_weights * cosine
...
def train_step(param_model, param_dataloader, param_optimizer, param_criterion, param_dataset_specific_models, param_weighting_mode, param_device):
    ...
    for batched_images, batched_masks, batched_dataset_ids in param_dataloader:
        ...
        cross_dataset_predictions, cross_dataset_features = param_model(batched_images, return_feature_maps=True)
        ...
        for yt, yp in zip(batched_masks, cross_dataset_predictions):
...
if __name__ == '__main__':
    ...
    if os.path.exists(RESUME_CHECKPOINT_PATH):
        start_epoch, best_validation_metric, num_epochs_no_improvement = load_resume_checkpoint(model, training_loss_function, optimizer, scheduler, RESUME_CHECKPOINT_PATH, DEVICE)

    for epoch in range(start_epoch, HYPERPARAMETERS['num_epochs']):
        start_time = time.time()
        train_sampler.set_epoch(epoch)

        train_loss, train_metrics = train_step(model, train_dataloader, optimizer, training_loss_function, dataset_specific_models, HYPERPARAMETERS['dsm_weight_mode'], DEVICE)
        validation_loss, validation_metrics = evaluate_step(model, validation_dataloader, dice_bce_loss_function, DEVICE)
        ...
        save_resume_checkpoint(model, training_loss_function, optimizer, scheduler, epoch, best_validation_metric, num_epochs_no_improvement, RESUME_CHECKPOINT_PATH)