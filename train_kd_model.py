...
def load_dataset_specific_models(dataset_specific_models_checkpoints, device):
    ...
    for dataset_id, checkpoint_path in dataset_specific_models_checkpoints.items():
        ...
        dataset_specific_model = freeze_model_parameters(dataset_specific_model)
...
def train_step(model, dataloader, optimizer, criterion, fused_model, device):
    model.train()
    fused_model.eval()
    ...
    for batched_images, batched_masks in dataloader:
        ...
        with torch.no_grad():
            _, fused_features = fused_model(batched_images, weighting_mode=None, dataset_ids=None, return_features=True)
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, HYPERPARAMETERS['num_epochs']):
        ...
        train_loss, train_metrics = train_step(model, train_dataloader, optimizer, criterion, fused_model, DEVICE)
