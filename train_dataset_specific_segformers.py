...
DATASET_NAME = 'bmshare' # 'bmshare', 'brats', 'brats_ped'
DATASET_PATH = f'{ROOT_PATH}/datasets/{DATASET_NAME}'

# Constant for model checkpoint path and log path
MODELS_AND_LOG_ROOT_PATH = f'{ROOT_PATH}/files/dataset_specific/segformers/mit-b2/v2/{DATASET_NAME}'
...
def load_segformer(model_name):
    #segformer_model = SegformerForSemanticSegmentation.from_pretrained(model_name, num_labels=len(ID2LABEL), id2label=ID2LABEL, label2id=LABEL2ID, ignore_mismatched_sizes=True)
    segformer_model = SegformerForSemanticSegmentation.from_pretrained(model_name, num_labels=1, ignore_mismatched_sizes=True)
    return segformer_model

# Function that performs a forward pass through the Segformer model and resizes the output logits to match the input image size
def segformer_forward(model, batched_images, return_features=False):
    segformer_outputs = model(pixel_values=batched_images, output_hidden_states=return_features)
    logits = F.interpolate(segformer_outputs.logits, size=batched_images.shape[2:], mode='bilinear', align_corners=False)
    
    if return_features:
        segformer_features = segformer_outputs.hidden_states
        return logits, segformer_features
...
def train_step(model, dataloader, optimizer, criterion, device):
    ...
    for batched_images, batched_masks in dataloader:
        ...
        y_pred = segformer_forward(model, batched_images)
...
if __name__ == '__main__':
    ...
    segformer_model = load_segformer(HYPERPARAMETERS['segformer_model_name']).to(DEVICE)
    optimizer = torch.optim.Adam(segformer_model.parameters(), lr=HYPERPARAMETERS['init_learning_rate'])
    ...
    for epoch in range(HYPERPARAMETERS['num_epochs']):
        ...
        train_loss, train_metrics = train_step(segformer_model, train_dataloader, optimizer, criterion, DEVICE)
        validation_loss, validation_metrics = evaluate_step(segformer_model, validation_dataloader, criterion, DEVICE)
    ...
    segformer_model.load_state_dict(torch.load(CHECKPOINT_PATH, map_location=DEVICE))
    test_loss, test_metrics = evaluate_step(segformer_model, test_dataloader, criterion, DEVICE)
